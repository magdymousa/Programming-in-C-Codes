...
int card_ptr_comp(const void * vp1, const void * vp2) {
  //  printf("card ptr compare\n");
  const card_t * const * cp1;
  const card_t * const * cp2;
...
suit_t flush_suit(deck_t * hand) {
  ...
  for(j = 0;j < 4;j++){
	...
	if((hand->cards)[i]->suit == j){ 
      ...
      if(most_suit[j] >= 5) return j;
    }
  return NUM_SUITS;
}

unsigned get_largest_element(unsigned * arr, size_t n) {
  //  printf("get largest element\n");
  size_t i = 0;
  unsigned max = arr[0];
  for(i = 1;i < n;i++){
    if(arr[i] > max){
...
unsigned * get_match_counts(deck_t * hand) {
  unsigned int * match_counts = malloc(hand->n_cards*sizeof(*match_counts));
