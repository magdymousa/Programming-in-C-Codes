...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  // int *ptr = malloc(400);
  board_t* nboard = (board_t *)malloc(sizeof(*nboard));
  nboard->board  = malloc(h*sizeof(int *));
  if(nboard->board == NULL) printf("can not alloc memory\n");
  //  else  printf("successful  alloc memory\n");
  for(int i = 0;i < h;i++){
    nboard->board[i] = malloc(w*sizeof(int));
    for(int j = 0;j < w;j++){
      nboard->board[i][j] = UNKNOWN;
    }
    if(nboard->board[i] == NULL) printf("can not alloc memory for %d\n",i);
    // else  printf("successful  alloc memory for %d\n",i);
  }
  nboard->width = w;
  nboard->height = h;
  nboard->totalMines = numMines;
  for(int i = 0;i < numMines;i++){
    #if DEBUG == 1
    printf("w= %d,h= %d,mine= %d\n",nboard->width,nboard->height,nboard->totalMines);
    printf("bS->%ld,PS->%ld\n",sizeof(ptr),sizeof(*nboard->board[0]));
    #endif
    addRandomMine(nboard);
  }
  /*
  printf("\n*********************************");
  printBoard(nboard);
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  // printf("count mines y = %d , x = %d\n",y,x);
  int mines_count = 0;
  int nx = 0,ny = 0;
  for(int dy = -1;dy <= 1;dy++){
    ny = y + dy;
    // printf("dy = %d,y = %d,ny = %d\n",dy,y,ny);
    if((ny >= 0) && (ny < b->height)){
      //  printf("dy = %d,y = %d,ny = %d\n",dy,y,ny);
      // printf("ny true\n");
      for(int dx = -1;dx <= 1;dx++){
	nx = x + dx;
	//	printf("dx = %d,x = %d,nx = %d\n",dx,x,nx);
	if((nx >= 0) && ( nx < b->width)){
	  // printf("ny = %d,nx = %d\n",ny,nx);
	  // printf("nx true\n");
	  if( IS_MINE(b->board[y+dy][x+dx])){
	    mines_count++;
	  }
	}
      }
